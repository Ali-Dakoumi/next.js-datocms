      ...
      query allPosts($limit: IntType) {
        posts: allPosts(first: $limit, orderBy:_firstPublishedAt_DESC) {
         seo: _seoMetaTags {
          ...
          _firstPublishedAt
          photos {
            responsiveImage(imgixParams: {fm: webp, q: "80", h: "600", w: "600"}) {
              ...imageFields
            }
          }
          author {
...
query searchPosts($search: String!) {
  posts : allPosts(filter: {title: {matches: {pattern: $search}}}) {
     title
     _firstPublishedAt
     photos {
       responsiveImage(imgixParams: {fm: webp, q: "80", h: "600", w: "600"}) {
...
query post($title: String) {
  post: post(filter: {title: {eq: $title}}) {
    ...
    _firstPublishedAt
    photos {
      responsiveImage(imgixParams: {fm: webp, q: "100", h: "2000", w: "2000"}) {
        ...imageFields
      }
    }
    author {
...
query cardQuery {
  card {
    image {
      webp: url(imgixParams: {fm: webp, q: 100,  w: 1000, h: 1000})
    }
  }
}
`
